...
- Added the launch parameter --interior-alpha-option, which controls the alpha channel of extracted interior maps: 0 — no alpha channel (RGB only, no processing); 1 — fade the empty border of interiors to transparent; 2 — raycast the cell and make areas without solid geometry transparent.
- Added the launch parameter --interior-disable-water, which disables water rendering on extracted interior maps.
